...
//@"1"),
	...
	public class SolverTest
	{
		...
		public void Case2() {
			long[] AS = { 1, 3, 5, 7, 9 };
			Assert.AreEqual(0, Util.BinarySearch(AS, 0));
			Assert.AreEqual(0, Util.BinarySearch(AS, 1));
			Assert.AreEqual(1, Util.BinarySearch(AS, 2));
			Assert.AreEqual(1, Util.BinarySearch(AS, 3));
			Assert.AreEqual(4, Util.BinarySearch(AS, 9));

			// 条件にマッチする要素が見つからない場合は AS.length が返される
			Assert.AreEqual(5, Util.BinarySearch(AS, 10));
			Assert.AreEqual(5, Util.BinarySearch(AS, 100));
		}
	}
